...
-- Állítsuk be, hogy a számokat át lehessen konvertálni:
ALTER SESSION SET NLS_NUMERIC_CHARACTERS = '.,';

--1. Készítsük egy hello world nevû programot, ami egy változó segítségével kiírja azt, hogy hello world! (dbms_output.put_line();)
...
create or replace procedure hello(mitirki varchar2)  is
begin
dbms_output.put_line('Hello '|| mitirki);
end;
/

execute hello(' Franciska ');


-- 1.5 azt írja ki amit beadunk neki
...
END;
/
-- így csak egy elemet ír ki --> kell ide is egy ciklus!
...
create or replace function kerulet(r in number) -- in (megadja, hogy milyen módban használjuk) - kívülrõl kap értéket, out: a paraméter arra lesz használva, hogy visszaadjon vmit a fv-en kívül
return number
is 
...
--7. Készítsen egy eljárást, ami egy adott típusú (bemenõ paraméter) komponensnek kiirja a kémiai formuláját
...
--(8) Írjunk egy anonim pl/sql szkriptet, amiben egy változóban megadott tábla egy másik változóban megadott paramétere különbözõ értékeinek számát kérdezzük le.
